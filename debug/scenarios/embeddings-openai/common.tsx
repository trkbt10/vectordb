...
import { createVectorLiteState } from "../../../src/vectorlite/create";
import { add, search } from "../../../src/vectorlite/ops/core";
import { deserializeVectorLite, serialize } from "../../../src/vectorlite/serialize";
...
function runSearch<TMeta>(
  ...
  if (strategy === "hnsw")
    return searchWithExpr(vl, q, expr, {
      k: 5,
      index: idx,
      hnsw: {
        mode: opts?.hnswFilterMode ?? "soft",
        bridgeBudget: opts?.hnswBridge ?? 32,
        seeds: opts?.hnswSeeds ?? "auto",
        seedStrategy: "random",
      },
    });
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const db = createVectorLiteState<Meta>({ dim: d0, metric, strategy, hnsw: hnswParams, ivf: ivfParams });
        const dbBF = createVectorLiteState<Meta>({ dim: d0, metric, strategy: "bruteforce" });
        const dbHNSW = createVectorLiteState<Meta>({ dim: d0, metric, strategy: "hnsw", hnsw: hnswParams });
        const dbIVF = createVectorLiteState<Meta>({ dim: d0, metric, strategy: "ivf", ivf: ivfParams });
        ...
        const queries: { q: string; desc: string; expr?: FilterExpr }[] = [
          ...
          },
          {
            ...
            expr: {
              should: [
                { key: "tags", match: "antarctica" },
                { key: "tags", match: "arctic" },
                { key: "category", match: "animal" },
              ],
              should_min: 1,
            },
          },
