...
describe('indexing/local_crush', () => {
  it('saves segments across shard directories and opens back', async () => {
    ...
    await persistIndex(vl, { baseName: 'db', crush: env.crush, resolveDataIO: env.resolveDataIO, resolveIndexIO: env.resolveIndexIO, segmented: true, segmentBytes: 1<<16, includeAnn: false })
    ...
    const vl2 = await openFromIndex<{ tag?: string }>({ baseName: 'db', crush: env.crush, resolveDataIO: env.resolveDataIO, resolveIndexIO: env.resolveIndexIO, rebuildIfNeeded: true })
