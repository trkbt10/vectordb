...
import { decodeMetric, decodeStrategy } from '../vectorlite/format'
...
export async function saveIndexing<TMeta>(vl: VectorLiteState<TMeta>, opts: SaveIndexingOptions): Promise<void> {
  ...
  await writePlacementManifest(opts.baseName, { segments: manifest.segments, crush: opts.crush }, { resolveIndexIO: opts.resolveIndexIO })
  // 3) Write catalog (index-only IO)
  await writeCatalog(opts.baseName, { dim: vl.dim, metricCode: encodeMetric(vl.metric), strategyCode: encodeStrategy(vl.strategy) }, { resolveIndexIO: opts.resolveIndexIO })
  // 4) Build index file (index-only IO)
  await writeIndexFile(vl, entries, { baseName: opts.baseName, resolveIndexIO: opts.resolveIndexIO, includeAnn: opts.includeAnn })
}
...
export async function openIndexing<TMeta = unknown>(opts: OpenIndexingOptions): Promise<VectorLiteState<TMeta>> {
  ...
  const idxU8 = await (async () => {
    try { return await opts.resolveIndexIO().read(`${base}.index`) } catch { return null }
  })()
  if (!idxU8) { return await rebuildIndexingFromData<TMeta>(opts) }

  const { header, entries, ann } = decodeIndexFile(idxU8)
  const vl0 = createVectorLiteState<TMeta>({ dim: header.dim, metric: decodeMetric(header.metricCode), strategy: decodeStrategy(header.strategyCode), capacity: header.count })
  ...
  const manifest = await (async () => {
    try { const m = await opts.resolveIndexIO().read(`${base}.manifest.json`); return JSON.parse(new TextDecoder().decode(m)) as { segments: { name: string; targetKey: string }[] } } catch { return null }
  })()
  if (manifest) { for (const s of manifest.segments) segTarget.set(s.name, s.targetKey) }
  for (const e of entries) {
    const t = segTarget.get(e.ptr.segment) ?? (() => { const { primaries } = crushLocate(e.id, opts.crush); const key = primaries[0] ?? 'local'; segTarget.set(e.ptr.segment, key); return key })()
    const seg = segReaders.get(e.ptr.segment) ?? (await (async () => { const io = opts.resolveDataIO(t); const r = await DataSegmentReader.fromFile(io, `${e.ptr.segment}.data`, e.ptr.segment); segReaders.set(e.ptr.segment, r); return r })())
  ...
  if (opts.rebuildIfNeeded !== false) {
    ...
    if (isHnswVL(vl0)) { return finalize(buildHNSWFromStore(vl0)) }
    if (isIvfVL(vl0)) { return finalize(buildIVFFromStore(vl0)) }
  }
  return finalize(vl0)
}

/** Rebuild a state from data segments using the index folder manifest. */
export async function rebuildIndexingFromData<TMeta = unknown>(opts: OpenIndexingOptions): Promise<VectorLiteState<TMeta>> {
  const base = opts.baseName
  ...
  const vl = createVectorLiteState<TMeta>({ dim: cat.dim, metric: decodeMetric(cat.metricCode), strategy: decodeStrategy(cat.strategyCode) })
  // load all
