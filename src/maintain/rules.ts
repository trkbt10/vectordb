...
export function ruleLargeDatasetBF<TMeta>(threshold = 10000): Rule<TMeta> {
  return ({ stats }) => {
    if (stats.strategy === 'bruteforce' && stats.n >= threshold) {
      return { code: 'bf.large-dataset', severity: 'warn', message: 'Large dataset on BF; consider HNSW/IVF for latency.' }
    }
    return null
  }
}
...
export function ruleHnswLowDegree<TMeta>(minAvgDeg = 4): Rule<TMeta> {
  return ({ stats }) => {
    if (stats.strategy === 'hnsw' && stats.hnsw && stats.hnsw.avgDeg < minAvgDeg) {
      return { code: 'hnsw.low-avg-degree', severity: 'info', message: 'HNSW avgDeg low; consider increasing M or efConstruction.' }
    }
    return null
  }
}
...
export function ruleIvfImbalance<TMeta>(factor = 2): Rule<TMeta> {
  return ({ stats }) => {
    if (stats.strategy === 'ivf' && stats.ivf) {
      ...
      for (const x of arr) if (x > factor * avg) return { code: 'ivf.imbalance', severity: 'info', message: 'IVF list imbalance; consider retraining centroids.' }
    }
    return null
  }
}
...
export function ruleHnswTombstone<TMeta>(ratio = 0.3): Rule<TMeta> {
  return ({ vl, stats }) => {
    ...
    if (r !== null && r > ratio) {
      return { code: 'hnsw.tombstone-high', severity: 'warn', message: 'HNSW tombstone ratio high; consider compaction.' }
    }
    return null
  }
}
