...
function makeIO(overrides: Partial<FileIO> = {}): FileIO {
  return {
    async read(p: string) { void p; throw new Error("not implemented"); },
    async write(p: string, d: Uint8Array | ArrayBuffer) { void p; void d; /* noop */ },
    async append(p: string, d: Uint8Array | ArrayBuffer) { void p; void d; /* noop */ },
    async atomicWrite(p: string, d: Uint8Array | ArrayBuffer) { void p; void d; /* noop */ },
    ...overrides,
  };
}
...
describe("client/indexing", () => {
  it("checkPlacement returns ok=false with plan when manifest mismatches crush", async () => {
    ...
    const indexIO = makeIO({ read: async (p) => { void p; return new TextEncoder().encode(JSON.stringify(manifest)); } });
  ...
  it("checkPlacement returns ok=true when manifest missing or read fails", async () => {
    const indexIO = makeIO({ read: async (_p: string) => { void _p; throw new Error("no file"); } });
