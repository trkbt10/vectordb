...
test("throws when cacheName is missing", () => {
  expect(() => createCacheStorageFileIO({ cacheName: "", urlPrefix: "https://test" })).toThrow("cacheName is required");
});

test("throws when urlPrefix is missing", () => {
  expect(() => createCacheStorageFileIO({ cacheName: "test", urlPrefix: "" })).toThrow("urlPrefix is required");
});
