...
describe("ops.tune", () => {
  it("returns suggestions with bounded recall", () => {
    ...
    for (let i = 0; i < 50; i++) {
      add(bf, i + 1, new Float32Array([1, 0, 0]), null);
    }
    for (let i = 0; i < 50; i++) {
      add(bf, 100 + i, new Float32Array([0, 1, 0]), null);
    }
    const h = createState({
      dim: 3,
      metric: "cosine",
