...
function addEq(c: BitmapAttrContainer, key: string, v: Scalar, id: number) { let m = c.eqMap.get(key); if (!m) c.eqMap.set(key, m = new Map()); let s = m.get(skey(v)); if (!s) m.set(skey(v), s = new Set()); s.add(id>>>0) }
function delEq(c: BitmapAttrContainer, key: string, v: Scalar, id: number) { const m = c.eqMap.get(key); if (!m) return; const s = m.get(skey(v)); if (!s) return; s.delete(id>>>0); if (s.size===0) m.delete(skey(v)) }
function addExists(c: BitmapAttrContainer, key: string, id: number) { let s = c.existsMap.get(key); if (!s) c.existsMap.set(key, s = new Set()); s.add(id>>>0) }
function delExists(c: BitmapAttrContainer, key: string, id: number) { const s = c.existsMap.get(key); if (!s) return; s.delete(id>>>0); if (s.size===0) c.existsMap.delete(key) }
