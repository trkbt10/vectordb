...
function lowerBound(arr: NumEntry[], x: number, strict: boolean): number {
  ...
  if (strict) {
    // eslint-disable-next-line no-restricted-syntax -- post-adjust index forward
    let i = l;
...
function upperBound(arr: NumEntry[], x: number, strict: boolean): number {
  ...
  if (strict) {
    // eslint-disable-next-line no-restricted-syntax -- post-adjust index backward
    let i = l;
...
function addEq(c: BasicAttrContainer, key: string, v: Scalar, id: number) {
  // eslint-disable-next-line no-restricted-syntax -- map slot initialization needs mutation
  let m = c.eqMap.get(key);
  if (!m) c.eqMap.set(key, (m = new Map())); // eslint-disable-next-line no-restricted-syntax -- set slot initialization needs mutation
  let s = m.get(skey(v));
...
function addExists(c: BasicAttrContainer, key: string, id: number) {
  // eslint-disable-next-line no-restricted-syntax -- set slot initialization needs mutation
...
export function setAttrs(idx: AttrIndex, id: number, attrs: Attrs | null): void {
  idx.setAttrs(id, attrs);
}
/** Get attributes for an id (or null). */
export function getAttrs(idx: AttrIndex, id: number): Attrs | null {
  return idx.getAttrs(id);
}
/** Remove all attributes for an id. */
export function removeId(idx: AttrIndex, id: number): void {
  idx.removeId(id);
}
/** Equality preselection for a key/value. */
export function queryEq(idx: AttrIndex, key: string, value: Scalar): Set<number> | null {
  return idx.eq(key, value);
}
/** Existence preselection for a key. */
export function queryExists(idx: AttrIndex, key: string): Set<number> | null {
  return idx.exists(key);
}
/** Numeric range preselection for a key. */
export function queryRange(idx: AttrIndex, key: string, r: Range): Set<number> | null {
  return idx.range(key, r);
}
