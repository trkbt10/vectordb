...
function parseValue(text: string, want: KV["type"]): { ok: boolean; value: unknown } {
  ...
  if (want === "boolean") {
    ...
    if (low === "true" || low === "false") {
      return { ok: true, value: low === "true" };
    }
    return { ok: false, value: false };
  }
  if (want === "null") {
    return { ok: text.trim().toLowerCase() === "null", value: null };
  }
  try {
    const v = JSON.parse(text);
    if (want === "object" && (v === null || Array.isArray(v) || typeof v !== "object")) {
    ...
    <Dialog open={true} title="Edit Meta" width={60}>
      ...
      {rows.map((r, idx) => {
        ...
        const status = (() => {
          ...
          if (!parsed.ok) {
            return <Text color="yellow"> type mismatch </Text>;
          }
