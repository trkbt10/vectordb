...
export async function openFromConfig(pathToConfig: string): Promise<ClientWithDatabase<Record<string, unknown>>> {
  ...
  if (!name) {
    throw new Error("index.name is required in config");
  }
  const storageKind = cfg.storage?.type;
  if (!storageKind) {
    throw new Error("storage.type is required in config (node|memory|opfs)");
  }
  function resolveStorage(): StorageConfig {
    ...
    if (storageKind === "opfs") {
      return { index: createOPFSFileIO(), data: () => createOPFSFileIO() };
    }
    const idxRoot = cfg.storage?.indexRoot;
    const datRoot = cfg.storage?.dataRoot;
    if (!idxRoot) {
      throw new Error("storage.indexRoot is required for node storage");
    }
    if (!datRoot) {
      throw new Error("storage.dataRoot is required for node storage");
    }
  ...
  function resolveExtra(): { database: VectorDBOptions } | { onMissing: () => Promise<never> } {
    if (hasDb) {
      return { database: cfg.database as VectorDBOptions } as const;
    }
    return {
      onMissing: async () => {
